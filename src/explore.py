...
from .data import compile_data, KatiwjkData, LunarsimData
...
def run_model(version,
    ...
    seg_target_paths = sorted([
        os.path.join("./data/lunarsim/bev_seg/", fname)
        for fname in os.listdir("./data/lunarsim/bev_seg/")
        if fname.endswith(".png")
    ])                                      
    loader = trainloader
    loader_katwijk = LunarsimData(seg_target_paths, "./data/lunarsim/imgs_synch/", (352, 128), 4, 6)
    ...
    counter = 0
    with torch.no_grad():
        for batchi, (imgs, rots, trans, intrins, post_rots, post_trans, binimgs) in enumerate(loader_katwijk):
            # print(imgs.shape)
            # # imgs = loader_katwijk.__getitem__(batchi)
            # # print(imgs.shape)
            ...
            for si in range(imgs.shape[0]):
                ...
                plt.legend(handles=[
                    ...
                    mpatches.Patch(color=(1.0, 0.0, 1.0, 1.0), label='Max output: {}'.format(out[si].squeeze(0).max())),
                ], loc=(0.01, 0.86))
                plt.imshow(out[si].squeeze(0), vmin=0, vmax=0.1, cmap='Blues')
                add_ego(bx, dx)
                
                imname = f'eval_lunarsim/eval{batchi:06}_{si:03}.jpg'
                # imname = "eval_lunarsim/{}.jpg".format(out[si].squeeze(0).max())
                print('saving', imname)
                plt.savefig(imname)
                counter += 1
